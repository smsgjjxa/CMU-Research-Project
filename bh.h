...
typedef struct block_head//Çø¿é½á¹¹
{
...
bool cpuc(const unsigned char *a, const unsigned char *b)//check if two hash valus are the same
{
	for (int i = 0; i < 32; i++)
	{
		if (a[i] != b[i])
...
bool b_cmp(BH *bnow)//verify block
{
	...
	if (cpuc(str, bnow->next->hpb))//correct
		return TRUE;
	else//error
	{
		return FALSE;
...
{
	for (int i = 0; i < (dif / 8); i++)//check if bytes in front are all 0
	{
		if (str[i] != 0)
		{
			return 0;
	...
	if (str[dif / 8] > front0)//check if the last byte is correct(meet the need of front 0)
	{
		return 0;
	}
	return 1;
